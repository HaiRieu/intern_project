...
static constexpr uint16_t batteryChargeStatusUuid   = 0x2a1a;
...
NimBLEHIDDevice::NimBLEHIDDevice(NimBLEServer* server) {
    ...
    m_batteryChargeStatusChr =
        m_batterySvc->createCharacteristic(batteryChargeStatusUuid, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::NOTIFY);
    NimBLE2904* batteryLevelDescriptor = m_batteryLevelChr->create2904();
    NimBLE2904* batteryChargeStatusDescriptor = m_batteryChargeStatusChr->create2904();
    batteryLevelDescriptor->setFormat(NimBLE2904::FORMAT_UINT8);
    batteryChargeStatusDescriptor->setFormat(NimBLE2904::FORMAT_UINT8);
    batteryLevelDescriptor->setUnit(0x27ad); // percentage
    batteryChargeStatusDescriptor->setUnit(0x27ad); // percentage
...
/**
 * @brief Set the battery charge status characteristic value.
 * @param [in] state The battery charge status (0: not charge, 1: charging).
 * @param [in] notify If true sends a notification to the peer device, otherwise not. default = false
 */
void NimBLEHIDDevice::setBatteryChargeStatus(uint8_t state, bool notify) {
    // m_batteryChargeStatusChr->setValue(&state, 1);
    // if (notify) {
    //     m_batteryChargeStatusChr->notify();
    // }
...
/**
 * @brief Get the battery charge status characteristic
 * @returns A pointer to the battery charge status characteristic
 */
NimBLECharacteristic* NimBLEHIDDevice::getBatteryChargeStatus() {
    return m_batteryChargeStatusChr;
